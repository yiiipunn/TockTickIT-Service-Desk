...
| AUTHZ-06 | Security/Authorization | BR-06, BR-30 / AC-04, AC-20–AC-21 | Session invalidation matrix | Logout/change/reset/deactivate revoke required sessions | `server/tests/lab-03/security.integration.test.ts`, `server/tests/lab-03/users-admin.api.test.ts` | Partial (logout, password-change, reset, and deactivation revocation passed; remaining matrix coverage is pending) |
...
| UI-02 | UI Component | FR-05 / AC-05 | Role shell and routes | Correct name/role/navigation; forbidden route blocked | `client/tests/lab-03/AppShell.test.tsx` | Partial (identity, role, logout, password restriction, Requester boundary, Staff Queue, and Administrator User Management navigation passed; later route coverage remains pending) |
...
| --- | --- | --- |
| Server tests | `cd server; npm test` | Issue 9 verification passed: 25 files, 188 tests. Focused User Management: 2 files, 13 tests; Lab 3: 17 files, 112 tests. Shared PostgreSQL integration files now execute serially to preserve deterministic fixtures. |
| Client tests | `cd client; npm test` | Issue 9 verification passed: 12 files, 93 tests. Focused User Management and App Shell: 2 files, 14 tests; Lab 3: 6 files, 57 tests. Client test files execute serially to avoid local runner timeout pressure. |
...
Planned tests: **61**. Passed: **39**. Partial: **7**. Failed: **0**. Pending: **15**. Full server and client regressions passed with zero failures.
