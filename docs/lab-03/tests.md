...
| AUTHZ-04 | Security/Authorization | FR-23, FR-25 / AC-17, AC-23 | Notes/Admin disclosure | Requester sees no notes; non-Admin sees no User data | `server/tests/lab-03/comments-notes.api.test.ts`, `server/tests/lab-03/authorization.api.test.ts` | Partial (Requester note privacy passed; User Management disclosure remains pending) |
...
| --- | --- | --- |
| Server tests | `cd server; npm test` | Issue 8 verification passed: 23 files, 175 tests. Focused Issue 8: 1 file, 14 tests; Lab 3: 15 files, 99 tests. New fixtures reuse seeded reference rows and clean up comments, notes, sessions, Ticket, and Users. |
| Client tests | `cd client; npm test` | Issue 8 verification passed: 11 files, 86 tests. Focused Requester and Staff Detail: 2 files, 27 tests; Lab 3: 5 files, 50 tests. |
...
Planned tests: **61**. Passed: **31**. Partial: **8**. Failed: **0**. Pending: **22**. Full server and client regressions passed with zero failures.
